(function($, window, document, undefined) {
    ...
    var Diamonds = function(customOptions) {
        ...
        var selector = this.options.itemSelector.replace(/[^#\.]*[#\.]/,'');
        this.placeholder = $("<span class='"+selector+"'></span>");
    ...
    Diamonds.prototype._groupIntoRows = function(items, maxDiamondsPerRow, hideIncompleteRow, forceSymmetricLayout) {
        // Max number of diamonds per row
        maxDiamondsPerRow = Math.max(this.options.minDiamondsPerRow, 0 + maxDiamondsPerRow);
        maxDiamondsPerRow = Math.min(this.options.maxDiamondsPerRow, maxDiamondsPerRow);
        ...
        // Hide incomplete rows or force symmetry on incomplete rows
        if(hideIncompleteRow || forceSymmetricLayout) {
            if(rows.hasOwnProperty(rows.length - 1) && rows[rows.length - 1].length < rows.length % 2 === 0 ? maxDiamondsPerRow - 1 : maxDiamondsPerRow) {
                if(hideIncompleteRow) rows.pop();
                else if(forceSymmetricLayout) this._centerRow(rows,maxDiamondsPerRow);
            }
        }
        return rows;
    };

    Diamonds.prototype._centerRow = function(rows,maxDiamondsPerRow) {
        var row = rows.pop();
        var length = row.length;
        var even = row.length % 2 === 0;
        var evenRows = rows.length % 2 === 0;
        var evenPerRow = maxDiamondsPerRow % 2 === 0;
        var max = Math.floor((maxDiamondsPerRow - length)/2);

        //move row to right using "empty" diamonds
        for(var c = 0; c < max; c++) {
            row.unshift(this.placeholder);
        }
        if(!even && evenRows == evenPerRow || even && evenRows != evenPerRow) {
            var first = row.pop();
            if(evenRows) row.unshift(this.placeholder);
        }
        rows.push(row);

        //if we have item left, need to repeat for it
        if(typeof first != "undefined") {
            rows.push([first]);
            this._centerRow(rows,maxDiamondsPerRow);
        }
    };
